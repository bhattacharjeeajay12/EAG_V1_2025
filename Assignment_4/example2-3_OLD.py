...
async def draw_rectangle(x1: int, y1: int, x2: int, y2: int) -> dict:
    ...
    try:
        ...
        # Optional: no need for GetSystemMetrics in single-monitor
        # primary_width = GetSystemMetrics(0)

        # Ensure Paint window is active
        # Ensure Paint window is active and in foreground
        if not paint_window.has_focus():
            paint_window.set_focus()
            time.sleep(0.2)

        # Click on the Rectangle tool — hardcoded coordinates, should work
        paint_window.click_input(coords=(0, 0))
        time.sleep(0.2)

        # Get the canvas area
        canvas = paint_window.child_window(class_name='MSPaintView')

        # Draw rectangle (REMOVED +2560 for single monitor)
...
#         canvas.press_mouse_input(coords=(x1+0, y1)) #2560
