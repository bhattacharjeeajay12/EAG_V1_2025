"""
- This script performs semantic search over a small corpus of jokes using Ollama embeddings (nomic-embed-text) and FAISS.
- Steps performed:
    - Defines a list of jokes with metadata.
    - Gets embeddings for each joke using Ollama's local API.
    - Creates a FAISS index and adds the embeddings.
    - Stores joke metadata for lookup.
    - Embeds a query and retrieves the top-3 most similar jokes from the index.
    - Prints the top matches with their metadata.
- Useful for demonstrating vector search with metadata and multi-result retrieval using Ollama embeddings.
"""
