"""
- This script performs advanced document chunking and semantic search using Ollama embeddings (nomic-embed-text) and FAISS.
- Steps performed:
    ...
    - Stores chunk metadata (document name, chunk text, chunk id).
    - Creates a FAISS index and adds all chunk embeddings.
    - Embeds a query and retrieves the top-3 most similar chunks from the index.
    - Prints the top matches with their document and chunk info.
- Useful for scalable document search and retrieval with chunk-level granularity using locally served Ollama embeddings.
"""
