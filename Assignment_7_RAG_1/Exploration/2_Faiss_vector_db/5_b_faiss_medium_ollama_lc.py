"""
- This script demonstrates semantic search and vector store management using LangChain, Ollama embeddings, and FAISS.
- Steps performed:
    - Defines a list of jokes with metadata and converts them to LangChain Document objects.
    - Uses OllamaEmbeddings to embed the documents.
    - Creates a FAISS index and adds the documents to a LangChain FAISS vector store.
    - Saves and loads the FAISS index from disk.
    - Adds new documents to the loaded vector store.
    - Performs similarity search for a query and prints the top matches from both the original and loaded vector stores.
- Useful for advanced vector store workflows, persistence, and integration with LangChain and Ollama.
"""
