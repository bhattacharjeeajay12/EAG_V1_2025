...
try:
    ...
    yaml_file_path = os.path.join("server", "config.yaml")
...
index_path = os.path.join("server", index_name)
...
def get_vector_store(index_name=index_name, embedder=embedder):
    ...
    path = os.path.join("url_rag", "server", index_name)
...
def get_retrieved_docs(query: str, k: int = 3) -> tuple[list[str], list[str]]:
    ...
    try:
        ...
        if docs:
            page_content = [doc.page_content[:200] for doc in docs]
            web_url = [doc.metadata.get("url", "") for doc in docs]
        else:
            page_content = []
            web_url = []
