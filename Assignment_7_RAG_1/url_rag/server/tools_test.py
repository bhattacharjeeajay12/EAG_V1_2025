...
# -- CONFIG --
INDEX_PATH = r"D:\Study\Programs\TSAI\EAG_V1\Assignment_7_RAG_1\url_rag\server\weburl_index\faiss_web_index.bin"
META_PATH = r"D:\Study\Programs\TSAI\EAG_V1\Assignment_7_RAG_1\url_rag\server\weburl_index\faiss_web_metadata.json"
...
def get_vector_store(index_path=INDEX_PATH, meta_path=META_PATH):
    """
    Loads the FAISS index and metadata for OpenAI embedding-based search.
...
def get_retrieved_docs(query: str, k: int = 1, index_path=INDEX_PATH, meta_path=META_PATH):
    """
    Given a query, returns the top-k (url, content) pairs from the FAISS index.
    """
    index, metadata = get_vector_store(index_path, meta_path)
    query_vec = get_embedding_openai(query).reshape(1, -1)
...
def get_vector_store(index_name=index_name, embedder=embedder):
    ...
    if not os.path.exists(path):
        ...
        alt_path = os.path.join(os.getcwd(),index_name)
...
def get_retrieved_docs(query: str, k: int = 1) -> tuple[list[str], list[str]]:
    """
...
if __name__ == "__main__":
    # Example usage
    try:
        query = "Complete MLOps Bootcamp With 10+ End To End ML Projects"
        k = 1
